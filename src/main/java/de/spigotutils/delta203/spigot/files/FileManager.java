...
public class FileManager {
  ...
  public FileManager(String filename) {
    ...
    file = new File(SpigotUtils.plugin.getDataFolder(), filename);
  }
   ...
   */
  public void create() {
    if (!SpigotUtils.plugin.getDataFolder().exists()) {
      SpigotUtils.plugin.getDataFolder().mkdir();
    }
    try {
      if (!file.exists()) {
        Files.copy(Objects.requireNonNull(SpigotUtils.plugin.getResource(filename)), file.toPath());
      }
